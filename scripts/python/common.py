...
def _force_utf8_stdio() -> None:
    """Ensure stdout/stderr are UTF-8 even when piped on Windows.

    Python uses the ANSI code page for redirected streams on Windows, so
    JSON payloads embedding template content (e.g. TASKS_TEMPLATE_CONTENT)
    crash with UnicodeEncodeError whenever the template contains non-ASCII
    text. Reconfigure the streams to UTF-8 before anything is written.

    An explicit PYTHONIOENCODING from the caller always wins: when it is
    set, the streams were configured deliberately (e.g. a test pinning a
    legacy code page) and must be left untouched.
    """
    ...
    for stream in (sys.stdout, sys.stderr):
        ...
        try:
            ...
            # Already-closed or exotic streams: leave them as they are.
            pass
