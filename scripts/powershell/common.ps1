...
# Force UTF-8 for redirected stdout/stderr so JSON payloads that embed
# template content (e.g. TASKS_TEMPLATE_CONTENT) survive non-ASCII text.
# Without this, Windows PowerShell 5.1 encodes piped output with the legacy
# OEM code page and replaces unmappable characters with '?'.
try {
    [Console]::OutputEncoding = New-Object System.Text.UTF8Encoding($false)
} catch {
    # Hosts without an interactive console may refuse the change; keep going.
}
