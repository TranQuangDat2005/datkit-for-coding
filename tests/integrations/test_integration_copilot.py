...
class TestCopilotCommandsMode:
    ...
    def test_specify_agent_resolves_active_spec_template(self, tmp_path):
        ...
        # The customized specify command resolves the active template through
        # the create-new-feature script instead of the preset CLI, but must
        # still reference the *active* template rather than a hardcoded path.
        assert "active `spec-template`" in content
    ...
    def test_setup_falls_back_to_bundled_command_template_without_preset_override(self, tmp_path):
        ...
        specify_file = tmp_path / ".github" / "agents" / "speckit.specify.agent.md"
        content = specify_file.read_text(encoding="utf-8")
        assert "Follow this execution flow:" in content
    ...
    def test_setup_uses_preset_command_override_when_present(self, tmp_path):
        ...
        assert "Follow this execution flow:" not in content
...
class TestCopilotSkillsMode:
    ...
    def test_skills_hook_sections_explain_dotted_command_conversion(self, tmp_path):
        """Generated skills must carry one dotted-command note per instruction.

        Mirrors the shared base-class invariant: notes are injected only for
        the English instruction marker, so a customized template set without
        that marker legitimately yields zero notes.
        """
        copilot = self._make_copilot()
        self._setup_skills(copilot, tmp_path)
