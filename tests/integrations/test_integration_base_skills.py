...
class SkillsIntegrationTests:
    ...
    def test_hook_sections_explain_dotted_command_conversion(self, tmp_path):
        ...
        The note is only injected for the English instruction marker
        ``- For each executable hook, output the following``. The customized
        core templates phrase hook instructions differently, so a template set
        without that marker legitimately yields zero notes — the invariant is
        that notes always match instructions 1:1, never more, never fewer.
        """
        i = get_integration(self.KEY)
        m = IntegrationManifest(self.KEY, tmp_path)
        i.setup(tmp_path, m)
