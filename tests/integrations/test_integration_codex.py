...
class TestCodexHookCommandNote:
    ...
    def test_hook_note_injected_in_skills_with_hooks(self, tmp_path):
        """Skills must carry one dotted-command note per hook instruction.

        The customized core templates phrase hook instructions without the
        English marker, so zero notes is valid — notes must match markers 1:1.
        The injected wording itself ("constructing command invocations") is
        covered by the pure-function tests below.
        """
