...
def _get_installed_version() -> str:
    """Return the installed spec-datkit distribution version or 'unknown'.
    ...
    try:
        return importlib.metadata.version("spec-datkit")
...
self_app = typer.Typer(
    name="self",
    help=(
        "Manage the specify CLI itself: report the installed version. "
        "Upgrade checks and self-upgrade from upstream are disabled in "
        "this build."
    ),
...
def self_check() -> None:
    """Report the installed spec-datkit version. Read-only.

    This build does not query GitHub for newer releases: spec-datkit is an
    independent distribution, so upstream spec-kit release tags are not
    meaningful here. Reinstall from your distribution source to update.
    """
    installed = _get_installed_version()
    console.print(f"Installed: {installed}")
    console.print(
        "[dim]Update checks against upstream GitHub releases are disabled "
        "in this spec-datkit build.[/dim]"
    )
...
def self_upgrade(
    dry_run: bool = typer.Option(
        False,
        "--dry-run",
        help="Ignored: self-upgrade is disabled in this build.",
    ),
    ...
    """Disabled: this build does not upgrade itself from upstream.

    spec-datkit is an independent distribution: upgrading from the upstream
    github/spec-kit repository would replace it with the original specify-cli
    package, so the self-upgrade path is intentionally removed. Reinstall
    manually from your distribution source to update.

    Exit codes:
      1      always — self-upgrade is disabled
    """
    console.print(
        "[red]Error:[/red] Self-upgrade is disabled in this spec-datkit build."
    )
    console.print(
        "Upgrading from the upstream github/spec-kit repository would "
        "replace this distribution with the original specify-cli package."
    )
    console.print(
        "Reinstall manually from your distribution source, e.g.:\n"
        "  uv tool install --force spec-datkit --from <your-source>"
    )
    raise typer.Exit(1)
